...
class HandelsRegister:
    ...
    def search_company(self):
        ...
        if self.args.force == False and cachename.exists():
            ...
            # # Assuming 'html' is the string you're writing to a file
            ...
            if (
                hasattr(self.args, "NiederlassungSitz")
                # hasattr(self.args, "niederlassung")
                and self.args.NiederlassungSitz != None
                # and self.args.niederlassung != None
            ):
                self.browser["form:NiederlassungSitz"] = self.args.NiederlassungSitz
                # self.browser["form:niederlassung"] = self.args.niederlassung
            if hasattr(self.args, "registerArt") and self.args.registerArt != None:
                self.browser["form:registerArt_focus"] = self.args.registerArt
            if (
                hasattr(self.args, "registerNummer")
                and self.args.registerNummer != None
            ):
...
def parse_args():
    ...
    parser.add_argument(
        ...
        required=True,
        default="Kloepfel Consulting GmbH",  # TODO replace default with a generic search term
    )
    parser.add_argument(
        "-so",
        "--schlagwortOptionen",
